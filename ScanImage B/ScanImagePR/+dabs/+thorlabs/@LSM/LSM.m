...
%         deviceTypeDescriptorSDK = 'Camera'; %Descriptor used by SDK for device type in function calls, e.g. 'Device', 'Camera', etc.
    ...
    properties
        ...
        framerate_user;
        fastZEnable; % PR2015-07-08
        offset_directly; % PR2015-10
        pockelsZoffset = 0;
        topbias = 0;
        leftbias = 0;
        pockelsZ = false;
        exec_after; % PR2015-10
        fastZScanType;
        fastz_step_nbplanes;
        fastz_step_stepsize;
        fastz_cont_nbplanes;
        fastz_cont_amplitude;
        fastz_step_settlingtime;
        highVal;
        lowVal;
        dutyCycleZ;
        zero_pos_Z;
        ...
        oct15_offset_voltage; % output, in V
        oct15_target_pos; % from Hall sensor, in V
        oct15_mode; % 1 = target position, 0 = set offset directly
...
%             delete(obj.waitbarHandle);
            ...
            if exist('obj.PRvoiceCoil')~=0
                obj.PRvoiceCoil.stop
            end
            if exist('obj.PRvoiceCoil_sense')~=0
                obj.PRvoiceCoil_sense.stop
            end
...
%                 amplitude = CRS_amplitude * obj.scanAngleMultiplierSlow/obj.crsAngle2VoltageFactor*obj.galvoAngle2VoltageFactor;
                amplitude = 1/doubling*CRS_amplitude * obj.scanAngleMultiplierSlow/obj.crsAngle2VoltageFactor*obj.galvoAngle2VoltageFactor*8/9;
            else
                amplitude = 0;
            end
            ...
            if obj.framerate_user_check
                fr = obj.framerate_user;
            else
                fr = obj.framerate;
            end
            
            fprintf('Scanning amplitudes. Galvo: %0.2f V, CRS: %0.2f V.\n',0.01*round(amplitude*100/doubling),0.01*round(CRS_amplitude*100));
            samplingFrequency = 5e5;
            numsamples = floor(samplingFrequency/ fr-10*30/fr); % empirical value to make sawtooth shorter than a frame
            numsamples = round(numsamples/doubling);
            effectiveFrequency = samplingFrequency/numsamples;
            if obj.galvoOffset ~= 0
                disp('Warning: Galvo offset is not zero.');
            end
            % if the frame rate is not at maximum:
            reduction = max(20,numsamples*(numsamples/samplingFrequency - obj.scanLinesPerFrame*obj.framePeriodMeasuredMean/2/doubling)/(numsamples/samplingFrequency));
            if obj.galvoOffset+amplitude < 10 && obj.galvoOffset-amplitude > -10
                sawtooth1 = linspace(-amplitude,amplitude,numsamples-reduction)'+obj.galvoOffset; % 35/5e5 is rougly one line (65 us) for the flyback of the galvo
                sawtooth2 = linspace(amplitude,-amplitude,reduction*doubling)'+obj.galvoOffset;
                if doubling == 2
                    sawtooth = [sawtooth1; sawtooth1; sawtooth2];
                else
                    sawtooth = [sawtooth1; sawtooth2];
                end
            else
                sawtooth1 = linspace(-amplitude,amplitude,numsamples-reduction)';
                sawtooth2 = linspace(amplitude,-amplitude,reduction*doubling)';
                if doubling == 2
                    sawtooth = [sawtooth1; sawtooth1; sawtooth2];
                else
                    sawtooth = [sawtooth1; sawtooth2];
...
%             obj.configureFrameAcquiredEvent('resume');
    ...
    methods (Hidden)
        ...
        function postflightAcquisition(obj)
            ...
            % PRVoiceCoil Motor control
            samplingFrequencyVC = 1e4;
            if obj.framerate_user_check
                fr = obj.framerate_user;
            else
                fr = obj.framerate;
            end
            numsamplesVC = floor(samplingFrequencyVC/obj.fastz_cont_nbplanes)-50;
            sawtoothVC = linspace(0,0,numsamplesVC)'; % 35/4e5 is roughly one 10 lines (650 us) for the flyback of the galvo

            if obj.fastZEnable
                if isempty(obj.PRvoiceCoil.deviceNames)
                    obj.PRvoiceCoil.createAOVoltageChan(obj.fastzDevice,[obj.fastzAOChannel obj.fastzAOPockels]); % fastzAOPockels
                    obj.voiceCoilCmdOutputRate = samplingFrequencyVC;
                    obj.PRvoiceCoil.cfgSampClkTiming(obj.voiceCoilCmdOutputRate,'DAQmx_Val_FiniteSamps',2);
                    obj.voiceCoilTriggerChannel = 0;
                    obj.PRvoiceCoil.cfgDigEdgeStartTrig(sprintf('PFI%d',obj.voiceCoilTriggerChannel)); % this is the frameClock2 input from Counter2 of board Dev1
                    obj.PRvoiceCoil.set('startTrigRetriggerable',1);
                end

                obj.PRvoiceCoil.stop();
                obj.PRvoiceCoil.cfgOutputBuffer(length(sawtoothVC));
                obj.PRvoiceCoil.cfgSampClkTiming(samplingFrequencyVC,'DAQmx_Val_FiniteSamps',length(sawtoothVC));

    %             obj.PRgalvo.cfgDigEdgeStartTrig(sprintf('PFI%d',obj.extFrameClockTerminal));
                obj.voiceCoilTriggerChannel = 0;
                obj.PRvoiceCoil.cfgDigEdgeStartTrig(sprintf('PFI%d',obj.voiceCoilTriggerChannel));
                obj.PRvoiceCoil.set('startTrigRetriggerable',1);
                obj.PRvoiceCoil.writeAnalogData([sawtoothVC sawtoothVC],false); % false = no autostart
                obj.PRvoiceCoil.start();
                pause(0.3);
            end
            obj.PRvoiceCoil.abort();
...
%             obj.PRgalvoPark.clear
    ...
    methods
        ...
        function clearDAQ(obj)
            ...
            obj.CRScmd.clear();
            obj.PRgalvo.clear();
            obj.PRvoiceCoil.clear();
            obj.PRvoiceCoil_sense.clear();
            obj.frameClock.clear();
...
%                     );
        ...
        function result = PRinitializeScanBoard(obj)
            ...
            % voice coil position sensor
            gTask = Task('PR Voice Coil Sense Task');
            obj.PRvoiceCoil_sense = gTask;
            obj.PRvoiceCoil_sense.createAIVoltageChan(obj.fastzDevice,obj.fastzAIHallSensor);
            
            % control voice coil motor
            vTask = Task('PR Voice Coil Control Task');
            obj.PRvoiceCoil = vTask;
            if obj.fastZEnable 
                obj.PRvoiceCoil.createAOVoltageChan(obj.fastzDevice,[obj.fastzAOChannel obj.fastzAOPockels]);
                obj.voiceCoilCmdOutputRate = 4e5;
                obj.PRvoiceCoil.cfgSampClkTiming(obj.voiceCoilCmdOutputRate,'DAQmx_Val_FiniteSamps',2);
                obj.voiceCoilTriggerChannel = 0;
                obj.PRvoiceCoil.cfgDigEdgeStartTrig(sprintf('PFI%d',obj.voiceCoilTriggerChannel)); % this is the frameClock2 input from Counter2 of board Dev1
                obj.PRvoiceCoil.set('startTrigRetriggerable',1);
            ...
            dutyCycle = 0.9995;
            if obj.framerate_user_check & obj.framerate_user < obj.framerate
...
%             obj.lineClock.start();
        ...
        function result = PRpreflight(obj)
            ...
            if ~strcmp(obj.acqState,'grab') && ~strcmp(obj.acqState,'loop')
                ...
                fprintf('Waiting for the resonant scanner to stabilize (1.5 seconds) ...\n');
                pause(1.5);
            end
            ...
            numsamples = floor(samplingFrequency/fr/doubling-10*30/obj.framerate); % empirical value to make sawtooth shorter than a frame
            effectiveFrequency = samplingFrequency/numsamples;
            % if the frame rate is not at maximum:
            reduction = max(20,numsamples*(numsamples/samplingFrequency - obj.scanLinesPerFrame*obj.framePeriodMeasuredMean/2/doubling)/(numsamples/samplingFrequency));
            if obj.galvoOffset ~= 0
...
%             obj.PRgalvo.cfgDigEdgeStartTrig(sprintf('PFI%d',obj.extFrameClockTerminal));
            ...
            if ~obj.fastZEnable
                ...
                effectiveplanes = obj.fastz_cont_nbplanes;
            end
            % PRVoiceCoil Motor control
            amplitudeVC = obj.fastz_cont_amplitude;
            samplingFrequencyVC = 1e4;
%             numsamplesVC = floor(samplingFrequencyVC/fr - 20)*obj.effectiveplanes; % empirical value to make sawtooth shorter than a frame
            numsamplesVC = floor(samplingFrequencyVC/(fr/effectiveplanes))-2;

            y = amplitudeVC;
            x = fr/effectiveplanes;
            
            filter1 = [55.0000   59.5955   63.7827   67.4265   70.4038   72.6107   73.9678   74.4258   73.9678   72.6107   70.4038 67.4265   63.7827   59.5955   55.0000]/1000;
%             filter2 = [43.9573   45.7512   47.4072   48.9053   50.2270   51.3556   52.2768   52.9785   53.4515   53.6896   53.6896  53.4515   52.9785   52.2768   51.3556   50.2270   48.9053   47.4072   45.7512   43.9573]/1000;
%             filter3 = [77.8599   91.3694  103.0187  111.5988  116.1533  116.1533  111.5988  103.0187   91.3694   77.8599]/1000;
...
%             d = 0.004472;
%             e = -0.1161;
%             LOW = a +b*y + c*y^2 + d*x +  e*x*y;
%  
%             a = 0.1838;
%             b = 2.841;
%             c = 10.88;
%             d = 0.0401;
%             HIGH = a +b*y + c*y^2 + d*x;
            HIGH = amplitudeVC;
            LOW = -HIGH/15;
%             sawtoothVC = linspace(LOW*0.5,LOW*1.5,numsamplesVC)';
            sawtoothVC = linspace(LOW*0.5,LOW*1.5,numsamplesVC)';
            step_duration = obj.dutyCycleZ; % additional, manual offset
            lastpart = round(numel(sawtoothVC)-samplingFrequencyVC*step_duration/1000):numel(sawtoothVC);
% % %             middlepart = round(numel(sawtoothVC)/2-samplingFrequencyVC*0.01255):numel(sawtoothVC)/2;
            sawtoothVC(lastpart-samplingFrequencyVC*0.002) = HIGH; %linspace(0.8*HIGH,HIGH,numel(lastpart));
%             sawtoothVC(1:numel(sawtoothVC)/2 -samplingFrequencyVC*0.000) = linspace(LOW,HIGH*0.1,numel(sawtoothVC)/2);
% % %             sawtoothVC(middlepart-samplingFrequencyVC*0.000) = -HIGH;
%             sawtoothVC = conv([sawtoothVC; sawtoothVC; sawtoothVC],filter1,'same');
%             sawtoothVC = sawtoothVC((numel(sawtoothVC)/3+1):numel(sawtoothVC)*2/3);
%             figure(414); plot(sawtoothVC)
%             [[max(sawtoothVC) min(sawtoothVC)] fr/effectiveplanes]
            if obj.fastZEnable
                disp(strcat('The 5 ms pulse amplitude is',32,num2str(HIGH),32,', the long stretch',32,num2str(LOW),32,'V.'));
            else
                sawtoothVC = zeros(floor(samplingFrequencyVC/fr)-2,1);
            end
            if isempty(obj.PRvoiceCoil.deviceNames)
                    obj.PRvoiceCoil.createAOVoltageChan(obj.fastzDevice,[obj.fastzAOChannel obj.fastzAOPockels]);
                    obj.voiceCoilCmdOutputRate = samplingFrequencyVC;
            ...
            % pockels part
            possibleRange = obj.beamCalibrationLUT(end) - obj.beamOnPowerVoltages;
            if obj.pockelsZ && obj.fastZEnable
                topup = obj.pockelsZoffset;
                topup = min(possibleRange,topup);
                pockelsVC = zeros(size(sawtoothVC));
                pockelsVC(1:min(lastpart)) = linspace(0,-topup,numel(1:min(lastpart)));
                pockelsVC(min(lastpart):end) = linspace(-topup,0,numel(pockelsVC(min(lastpart):end)));
                pockelsVC = - pockelsVC + min(pockelsVC);
            else
                 pockelsVC = zeros(size(sawtoothVC));
            end
            
            plane_counter = effectiveplanes;
            XperFrame = floor(numel(pockelsVC)/plane_counter);
            if ~obj.fastZEnable; plane_counter = 1; end
            for i = 1:plane_counter
                pockelsVC(((i-1)*XperFrame+1):i*XperFrame) = pockelsVC(((i-1)*XperFrame+1):i*XperFrame) + linspace(-obj.topbias,obj.topbias,XperFrame)';
            end
%                 figure(999), plot(pockelsVC')
%                figure, plot(sawtoothVC)
...
%             obj.PRgalvo.cfgDigEdgeStartTrig(sprintf('PFI%d',obj.extFrameClockTerminal));
            ...
            obj.PRvoiceCoil.cfgDigEdgeStartTrig(sprintf('PFI%d',obj.voiceCoilTriggerChannel));
            obj.PRvoiceCoil.set('startTrigRetriggerable',1);
            obj.PRvoiceCoil.writeAnalogData([sawtoothVC pockelsVC],false); % false = no autostart
            obj.PRvoiceCoil.start();
...
%                 return
            ...
            if obj.loggingEnable && (obj.focusSave || ~strcmpi(obj.acqState,'focus')) && ~obj.write2RAM
                if obj.savefast
