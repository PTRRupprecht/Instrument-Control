classdef MachineDataFile < most.DClass
    %MACHINEDATAFILE Abstract class implementing concept of Machine Data
    % Classes that inherit from MachineDataFile will have a data field
    ...
    % ...
    %
    % Here MyClass1 and MyPack.MyClass2 inherit from MachineDataFile. When
    ...
    % initialization of .mdfData occurs during the MachineDataFile
    % constructor.
    %
    % To use the machine data file in a class, first derive from
    % MachineDataFile. Then specify the mdfClassName for your class, which
    ...
    %%% MDF spec %%%
    % Defns: 'MDFc' is the MachineDataFile class. 'MDF' is typically the
    % machine data file itself.
    %
    %% Getting an MDF 
    % When MachineDataFile requires an MDF, it looks to see if one is
    ...
    properties (Abstract,Constant,Hidden)
        ...
        % structure, MachineDataFile initializes properties of the object
        % being instantiated. Defaults to false.
        
        mdfPropPrefix;
        % String. Applies only if mdfDirectProp is true. This string is a
        % prefix for properties to be initialized by MachineDataFile. For
        % example, if mdfPropPrefix is 'pre', and the variables in the
        % Machine Data File are 'p1' and 'p2', MachineDataFile will
    ...
    methods (Access = protected)
        function obj = MachineDataFile()
            ...
            varstruct = obj.getMDFVars; % can throw
    ...
    methods (Static)
        ...
        function updateMachineDataFile(machineDataFile)
            ...
            most.DClass.setClassDataVarStatic(...
                            'most.MachineDataFile',...
    ...
    methods (Hidden)
        ...
        function vars = getMDFVars(obj)
            ...
            assert(~isempty(obj.mdfHeading));
            
            mdf = most.MDF.getInstance();
            status = most.MachineDataFile.ensureMDFExists;
            switch status
                case {'exist' 'spec'}
    ...
    methods (Static,Hidden)        
        ...
        function status = ensureMDFExists
            mdf = most.MDF.getInstance;
            ...
            [tf specOrCreate path name] = zlclUserSpecifiesOrCreatesMDF;
            if tf
        ...
        function tf = ensureClassAndDependentsAreInMDF(mdfCls)
            ...
            allClasses = most.MachineDataFile.getAllDependentClasses(mdfCls);
            mdf = most.MDF.getInstance;
...
function [tfsuccess specOrCreate mdfPath mdfName] = zlclUserSpecifiesOrCreatesMDF
...
if tflastmdf
    ...
    if isequal(mdfName,0) || isequal(mdfPath,0) % user canceled or killed dlg
        ...
        switch resp
            case 'Create new file'
                ...
                if mdfPath
                    [~,mdfName] = most.MDF.createNewMDF(mdfPath);
                    specOrCreate = 'create';
                    tfsuccess = true;
                else
                    % none; failure
    ...
    switch resp
        ...
        otherwise % 'Cancel' or kill dlg window
            % none; failure
    end    
end
...
most.DClass.ensureClassDataFileStatic(...
    'most.MachineDataFile',struct('lastMachineDataFilePath',''));

lastmdfpath = most.DClass.getClassDataVarStatic(...
    'most.MachineDataFile','lastMachineDataFilePath');
...
function fname = zlclGetModelFullFileName(clsName)
    ...
    fname = fullfile(most.DClass.classPrivatePathStatic(clsName),[strippedClsName '_model.m']);
end
