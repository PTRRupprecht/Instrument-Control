classdef TifStream < handle
    ...
    methods
        function obj = TifStream(filename, frameWidth, frameHeight, bitdepth, varargin)
    ...
    methods (Access = private)
        ...
        function byteArray = makeByteArray(obj,val,numBytes)
            ...
            for i=1:length(val)
                %Limits checking
                if val(i) > 2^(numBytes*8)-1 && val(i)
                    error(['Value (' val(i) ') exceeds that allowed for specified number of bytes, 4 GB.']);
%                       fprintf('Warning: This file will be bigger than 4 GB.');
                end
                %val(i) = uint32(i);
                val(i) = double(val(i));
