classdef MDF < handle
    %MDF Machine Data File reader/writer class.
    % MDF knows how to read/write from/to a Machine Data File.
    %
    % Usage:
    %   mdf = MDF.getInstance();
    ...
    methods
        ...
        % Parse/load an existing MDF. If fname is not an existing file,
        ...
        % loaded MDF. If successful, tf is returned as true and s is a
        ...
        % This updates the current MDF (both on disk and in memory) so that
        % variable varName under heading hding has the value val. An MDF
        ...
        % created in the MDF, false otherwise.        
        function tfNewVar = writeVarToHeading(obj,hding,varName,val)
            ...
            most.MDF.write(obj.fMDFName,obj.fHData);            
            
            tfNewVar = ~tfFound;
        end
        
        % This copies the contents of modelFile into the file on disk for
        % the currently loaded MDF. The new MDF is *not* reloaded
        % automatically.
    ...
    methods (Static)
        ...
        % Create a new (blank) MDF in mdfpath and return the full filename.
        function [fullfname shortfname] = createNewMDF(mdfpath)
            
            shortfname = most.MDF.getDefaultMDFName;
    ...
    methods (Static,Access=private)
        ...
        function hdingData = read(mdfname)
            % This function reads an MDF and produces the data structure
            % hdingData. hdingData is a struct array, with one element per
            % "heading block" in the MDF. The contents of the MDF that
