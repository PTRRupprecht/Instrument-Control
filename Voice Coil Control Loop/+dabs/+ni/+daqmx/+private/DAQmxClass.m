...
relPathArgs = {'national instruments','ni-daq','daqmx ansi c dev','include'};
p64 = fullfile(most.idioms.startPath,'program files (x86)',relPathArgs{:});
p32 = fullfile(most.idioms.startPath,'program files',relPathArgs{:});

if exist(p64,'dir')
    p = p64;
elseif exist(p32,'dir')
    p = p32;
else
    error('Unable to find installation copy of DAQmx driver header file');
end
