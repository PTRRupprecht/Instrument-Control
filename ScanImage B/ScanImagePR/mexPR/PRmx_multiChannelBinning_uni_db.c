...
unsigned __stdcall ThreadFunc( void* pArguments )
{
    ...
    for(ii=ini*nb_pxls; ii<fin*nb_pxls; ii++,ii2++) {
        ...
        if ((ii % (2*nb_pxls)) >= nb_pxls) {
            ...
            y1[jj] = ch1/binning/4; // division by 4 accounts for bitshift by 2 from 14bit to 16bit conversion
            y1[jj2] = ch2/binning/4;
            y1[jj-nb_pxls] = ch1/binning/4; // division by 4 accounts for bitshift by 2 from 14bit to 16bit conversion
            y1[jj2-nb_pxls] = ch2/binning/4;

        }
        else {
            hh = hh + binning*nb_channels;
        }
    }
    _endthreadex( 0 );
    return 0;
...
{    
    ...
    yArray1 = mxCreateNumericMatrix(nb_pxls, 2*nb_lines, mxDOUBLE_CLASS,mxREAL);
