...
{
    int ini,fin,ii,kk,hh,jj,jj2,temp;
    uint16_T *ima;
    double *y1, ch;
    int nb_lines,nb_pxls,nb_channels,binning,NdelayedChannels;
    myargument arg;
    ...
    nb_pxls=arg.nb_pxls;
    binning=arg.binning;
    NdelayedChannels = arg.NdelayedChannels;
    
    // indexing optimized by Adrian Wanner
    hh = ini*nb_pxls*binning*NdelayedChannels;
    for(ii=ini*nb_pxls; ii<fin*nb_pxls; ii++) {
        ...
        ch = ima[hh++];
        for (kk=1;kk<binning*NdelayedChannels;kk++) {
            ch += ima[hh++];
        }
        y1[jj] = ch/binning/4/NdelayedChannels; // division by 4 accounts for bitshift by 2 from 14bit to 16bit conversion
    }
...
{    
    int i,Nthreads,ini,fin,NdelayedChannels;
    uint16_T *ima;
    double *y1;
    ...
    NdelayedChannels = (int) mxGetScalar(prhs[6]);
	...
	// Make Thread Structure
        ...
        ThreadArgs[i].NdelayedChannels = NdelayedChannels;
